...
void C_Money();                 //中文存钱业务
void C_Money1();                //中文存钱失败选项
void C_Money2();                //中文存钱成功选项
void C_Withdraw_Money();               //中文取钱业务
void C_Withdraw_Money1();              //中文取钱失败选项
void C_Withdraw_Money2();              //中文取钱成功选项
void C_Transfer_Money();               //中文转钱业务
void C_Transfer_Money1();              //中文转钱失败选项
void C_Transfer_Money2();              //中文转钱失败选项
void C_Enquiry();                      //中文查询业务
void C_Enquiry_balance();              //中文查询余额
...
void C_Easy_Money1();           //中文简洁存钱成功选项
void C_Easy_Money2();           //中文简洁存钱失败选项
void C_Easy_Withdraw_Money1();         //中文简洁取钱成功选项
void C_Easy_Withdraw_Money2();         //中文简洁取钱失败选项
void C_Easy_Transfer_Money1();         //中文简洁转钱成功选项
void C_Easy_Transfer_Money2();         //中文简洁转钱失败选项
...
void CE_Deposit_Transaction();        //存钱交易
void CE_Withdraw_Transaction();       //取钱交易
...
{
	printf("\n\n\t是否继续注销该账户？\n\n\t1.注销前请确认余额是否清0，如果账户余额未清0，账号将无法注销成功。\n\n\t\t2.如果继续注销账号将会被回收，届时您将无法再次登录该账号。");
	int aaa, k1 = 1;
	while (k1)
	{
		k1 = 0;
...
void CE_ChooseBank()
{
	...
	while (k)
	{
		...
		else
		{
			...
			printf("\n\n\t\t2Agricultural Bank of China Bank\n");
			printf("\n\n\t\t3postal bank Bank\n");
			printf("\n\n\t\t4Any Bank\n");
			printf("\n\n\t\t0Exit the system\n");
...
void C_Easy_Main_Menu()
{
	...
	int index = 0;
	char ch;
	string select[] = { "【1】存钱","【2】取钱","【3】转帐","【4】退出系统","","","方向键 ↑ ↓ 选择功能，回车键确定" };
	string str = "▲ ";
	gotoxy(5, 1);
...
void C_Easy_Money1()
{
	int index = 0;
	char ch;
	string select[] = { "【1】重新输入金额数额","【2】返回主页","【3】退出系统","","","方向键 ↑ ↓ 选择功能，回车键确定" };
	string str = "▲ ";
	gotoxy(5, 1);
	//printf("方向键 ↑ ↓ 选择功能，回车键确定。\n");
	for (int i = 0; i < 6; i++)
...
void C_Withdraw_Money()
{
	...
	if (a % 100 || a > 10000)
	{
		...
		if (Sin == 1)
		{
			if (a > 10000)
			{
				printf("单笔取钱不得超过10000。\n\n");
			}
			else
				printf("金额数额须为100的整数。\n\n");
			C_Withdraw_Money1();//取钱失败选项
		}
		else if (Sin == 3)
		{
			if (a > 10000)
			{
				...
				printf("\n\n\n\n\n\n\n\n\n\t金额数额须为100的整数。\n\n");
			C_Easy_Withdraw_Money1();//取钱失败选项，简洁
		}
	}
	else
	{
		...
		else
		{
			...
			{
				printf("\n\t取钱成功!!!!\n");
				C_Withdraw_Money2();//取钱成功选项
			}
			else if (Sin == 3)
			{
				printf("\n\n\n\n\n\n\n\n\n\t取钱成功!!!!\n");
				C_Easy_Withdraw_Money2();//取钱成功选项，简洁
			}
		}
	}
}
...
void C_Transfer_Money()
{
	...
	if (findAccount3(TT))
	{
		...
		if (Sin == 1)
		{
			...
			printf("\t----------------------------------------------------\n");
			printf("\n\n\t\t>>请输入你要转出的金额数额：______\b\b\b\b\b\b");
		}
		else if (Sin == 3)
		{
			printf("\n\n\t\t>>请输入你要转出的金额数额：______\b\b\b\b\b\b");
